...
<ProtectedRoute>
<div class="pt-2">
  <h2 class="text-2xl font-bold text-center tracking-tight">Purchase Order Extractor</h2>
</div>
<div class="flex justify-center items-center h-[73vh] px-4">
  <div class="w-full max-w-4xl space-y-8">
    <div
      class="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer bg-muted hover:bg-muted/80 transition-all duration-300 shadow-lg hover:shadow-xl"
      role="button"
    ...
    >
      ...
      >
        <div class="flex flex-col items-center justify-center pt-5 pb-6">
          <svg
            class="w-12 h-12 mb-4 text-primary animate-bounce"
            aria-hidden="true"
          ...
          <p class="text-xs text-muted-foreground">
            Only PDF or ZIP files (Limit: 200 files)
          </p>
        </div>
        <input id="dropzone-file" type="file" accept=".zip,.pdf" class="hidden" on:change={handleFileSelect} />
      </label>
    </div>
    <div class="w-full flex flex-col space-y-2 items-start">
      <Label for="document">Upload Document</Label>
      <Input id="document" type="file" accept=".zip,.pdf" class="w-full" on:change={handleFileSelect} />
