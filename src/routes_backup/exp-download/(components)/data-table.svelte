<script lang="ts">
  ...
  function handleFilterInput(event: Event, cellId: string) {
    ...
    const currentFilterValues = get(tableModel.pluginStates.colFilter.filterValues);
    tableModel.pluginStates.colFilter.filterValues.set({
      ...currentFilterValues,
      [cellId]: input.value // Update the filter value for the given column
    });
  }
                      ...
                      <Table.Head {...attrs}>
                        ...
                        <!-- Add search input field to each header except for SL, View Logs, and checkbox -->
                            ...
                            <div class="flex flex-col space-y-1 pb-3">
                              <Input
                                placeholder="Search"
                                class="h-6 w-[150px] placeholder:text-muted-foreground/50"
                                type="search"
                                value={get(tableModel.pluginStates.colFilter.filterValues)[cell.id] || ''}
                                on:input={handleInputEvent(cell.id)} 
                              />
                            </div>
                          {/if}
